...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        with open(os.path.join(instance_data_root, "coco_data_.pkl"), "rb") as h:
        ...
        for data in pkl_data:
            for region in data["regions"]:
                ...
                caption = self.remove_special_characters(caption) # remove all special characters
                caption_words = caption.split(" ")
                bbox= []
                phrases = []
                for noun in region["nouns"]:
                    noun = self.remove_special_characters(noun)
                ...
                self.instance_images_path.append((os.path.join(instance_data_root, 'images/train2017', data['image_id']) ,caption, region_mask, bbox, Pharse2idx(caption, phrases)))
    ...
    def remove_special_characters(self, caption):
        cleaned_sentence = re.sub(r'[^a-zA-Z0-9\s]', '', caption)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                reg_loss = 10*regularizer(batch["bbox"], batch["p2i"], 16, device=accelerator.device)
                ...
                mask = batch["mask"]
                mask = mask.view(mask.shape[0] // mask.shape[1] , 1, mask.shape[1], mask.shape[1]) ## -> shape (bsz, 1, 64, 64)
                loss = F.mse_loss(model_pred.float(), target.float(), reduction="none") ## -> shape (bsz, 4, 64, 64)
