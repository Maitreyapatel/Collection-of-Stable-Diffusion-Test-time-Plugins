...
from utils.ptp_utils import AttentionStore, aggregate_attention, register_attention_control, only_SAR, only_CAR

logger = logging.get_logger(__name__)

class AttentionRefocusPipeline(StableDiffusionPipeline):
    r"""
    ...
    def get_backward_guidance_loss(self, attention_maps: List[List[List[torch.Tensor]]], bbox: Union[int, List[float]], object_positions, attention_res: int = 16, smooth_attentions: bool = False, kernel_size: int = 3, sigma: float = 0.5) -> torch.Tensor:
        ...
        self_attentions = attention_maps[0]
        cross_attentions = attention_maps[1]

        self_loss = caculate_loss_self_att(self_attentions, bbox)
        cross_loss = caculate_loss_cross_att(cross_attentions, bbox, object_positions, smooth_attentions, kernel_size, sigma)

        return self_loss + cross_loss
    ...
    def _compute_loss(self, attention_maps: List[List[List[torch.Tensor]]], bbox: Union[int, List[float]], object_positions, attention_res: int = 16) -> torch.Tensor:
